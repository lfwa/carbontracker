...
class EnergiDataService(IntensityFetcher):
    ...
    def _emission_current(self):
        url_creator = lambda area : f"""https://api.energidataservice.dk/datastore_search_sql?sql=SELECT co2."CO2Emission" from "co2emis" as co2 WHERE co2."PriceArea" = '{area}' ORDER BY co2."Minutes5UTC" DESC LIMIT 1"""
        areas = ["DK1", "DK2"]
        carbon_intensities = []
        ...
        for area in areas:
            ...
            carbon_intensities.append(response["result"]["records"][0]["CO2Emission"])

        return np.mean(carbon_intensities)
    
    def _emission_prognosis(self, time_dur):
        from_str, to_str = self._interval(time_dur=time_dur)
        url = f"""https://api.energidataservice.dk/datastore_search_sql?sql=SELECT co2."CO2Emission" from "co2emisprog" as co2 WHERE co2."Minutes5UTC" > timestamp'{from_str}' AND co2."Minutes5UTC" < timestamp'{to_str}' ORDER BY co2."Minutes5UTC" DESC"""
